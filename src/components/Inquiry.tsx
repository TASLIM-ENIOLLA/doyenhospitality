export default function Page() {
	return (
		<section className="py-20">
			<div className="container space-y-10">
				<div className="max-w-[600px] mx-auto">
					<div className="space-y-7">
						<div className="text-3xl md:text-4xl lg:text-5xl capitalize font-semibold leading-snug text-center">
							If You Have Any Question, Feel Free to Call 123-456-7890
						</div>
						<div className="capitalize text-lg font-normal text-gray-600 text-center">
							Ac cum nunc vitae ut turpis praesent nunc odio lorem dictumst vestibulum sem pharetra urna pretium.
						</div>
						<div className="text-center">
							<button className="px-10 py-4 uppercase text-lg font-semibold rounded-md text-white bg-gradient-to-r from-blue-600 to-blue-800">
								let&#39;s work together
							</button>
						</div>
					</div>
				</div>
			</div>
		</section>
	);
}